...
- Test Spec: `openteam/test.md`（当前为空，按 task.md 测试用例执行静态核对）
...
- [ ] 验收标准 A：新增 Doubao/DashScope realtime transformer 并接入 `TransformerMux`
  - 检查方法：
    - 静态检查是否新增：
...
- [ ] 验收标准 B：统一输出事件语义，避免上层按 provider 分叉
  - 检查方法：
...
- [ ] 验收标准 C：会话生命周期、EOF/EoS、错误传播、close 语义与设计一致
  - 检查方法：
...
- [ ] 不应提交文件检查（基于 `git diff --name-only`）
