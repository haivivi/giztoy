...
///
/// The random hyperplanes are generated using a seeded PCG RNG
/// with Box-Muller normal distribution. The seed and algorithm must
/// match Go's `rand.NewPCG(seed, seed^0xdeadbeef)` + `NormFloat64()`.
///
/// **Fallback**: If PCG cross-language consistency cannot be achieved,
/// planes can be loaded from a shared testdata JSON file generated by Go.
pub struct Hasher {
    dim: usize,
    bits: usize,
...
impl Hasher {
    ...
    ///
    /// Use this when loading planes from a shared testdata file for
    /// cross-language consistency.
