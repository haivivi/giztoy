...
use giztoy_minimax::Client;

use crate::Cli;

const APP_NAME: &str = "minimax";
    ...
    match cfg.resolve_context(cli.context.as_deref()) {
        ...
        None => {
            if cli.context.is_none() {
                anyhow::bail!(
                    "no context specified. Use -c flag or set a default context with 'minimax config use-context'"
                );
            }
...
/// Creates a MiniMax API client from context configuration.
