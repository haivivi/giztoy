...
/// Update async context in thread-local storage.
pub fn update_async_context(
    async_mode: bool,
    completed_tx: Option<tokio::sync::mpsc::UnboundedSender<(u64, HttpResponse)>>,
    next_req_id: Arc<AtomicU64>,
    pending_reqs: Arc<Mutex<HashSet<u64>>>,
) {
    ASYNC_MODE.with(|am| *am.borrow_mut() = async_mode);
    COMPLETED_TX.with(|tx| *tx.borrow_mut() = completed_tx);
    NEXT_REQ_ID.with(|id| *id.borrow_mut() = Some(next_req_id));
    PENDING_REQS.with(|pr| *pr.borrow_mut() = Some(pending_reqs));
}
...
impl Runtime {
    ...
    pub fn register_http(&mut self) -> Result<(), Error> {
        ...
        self.state
            .register_func("__builtin_http", |state| builtin_http(state))?;
...
fn builtin_http(state: &State) -> i32 {
    ...
    if async_mode && can_yield {
        ...
        if let (Some(tx), Some(req_id_counter), Some(pending)) =
            (completed_tx, next_req_id, pending_reqs)
        {
