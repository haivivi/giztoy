...
impl Runtime {
    ...
    pub fn register_require(&mut self) -> Result<(), Error> {
        ...
        let libs_dir =
            std::fs::canonicalize(&self.libs_dir).unwrap_or_else(|_| self.libs_dir.clone());
        ...
        self.state.register_func("require", move |state| {
            ...
            // Check if already loaded - return from cache
            // First, try to get from __loaded table
            let escaped_name = escape_lua_string(&name);
            let check_code = format!("return __loaded[\"{}\"]", escaped_name);
            if state.do_string(&check_code).is_ok() {
                // Check if result is not nil
                if !state.is_nil(-1) {
                    // Module already loaded, return cached value
                    return 1;
                }
                state.pop(1); // Pop nil result
            }

            // Check if currently loading (circular require detection)
            {
                let loaded_guard = loaded.lock().expect("loaded mutex poisoned");
                if loaded_guard.get(&name).copied().unwrap_or(false) {
                    // Module is being loaded right now - this is a circular require
                    eprintln!(
                        "[luau] warning: circular require detected for module '{}'",
                        name
                    );
                    state.push_nil();
                    return 1;
            ...
            if let Some(bytecode) = bytecode {
                ...
                if let Err(e) = state.load_bytecode(&bytecode, &name) {
                    eprintln!(
                        "[luau] require: failed to load bytecode for '{}': {}",
                        name, e
                    );
                    state.push_nil();
                    return 1;
...
fn validate_module_name(name: &str) -> Result<(), String> {
    ...
    if name.contains("..") {
        return Err(format!(
            "module name '{}' contains path traversal sequence",
            name
        ));
    }
    ...
    if name.starts_with('/') || name.starts_with('\\') {
        return Err(format!(
            "module name '{}' starts with path separator",
            name
        ));
    }
