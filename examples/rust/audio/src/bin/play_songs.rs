...
fn main() {
    ...
    while i < args.len() {
        let arg = &args[i];
        match arg.as_str() {
            "--list" | "-l" => list_songs = true,
            "--loop" => loop_play = true,
            "--help" | "-h" => {
                print_help();
                return;
            }
            _ if arg.starts_with("--song=") => {
                song_id = arg.strip_prefix("--song=").map(|s| s.to_string());
            }
            "--song" | "-s" => {
                i += 1;
                if i < args.len() {
                    song_id = Some(args[i].clone());
                }
            }
            _ if arg.starts_with("--volume=") => {
                if let Some(v) = arg.strip_prefix("--volume=") {
                    volume = v.parse().unwrap_or(0.5);
                }
            }
            _ if arg.starts_with("--output=") || arg.starts_with("-o=") => {
                output_file = arg.split('=').nth(1).map(|s| s.to_string());
            }
    ...
    // Get playlist
    let playlist = if let Some(ref id) = song_id {
        match Song::by_id(id) {
            Some(song) => vec![song],
            None => {
    ...
    loop {
        ...
        for (i, song) in playlist.iter().enumerate() {
            ...
            let wav_path = output_file.clone().unwrap_or_else(|| {
                ...
                tmp_dir
                    .join(format!("rust_song_{}.wav", song.id))
                    .to_string_lossy()
                    .to_string()
            });
            ...
            let play_result = if cfg!(target_os = "macos") {
                ...
                eprintln!(
                    " (no player available, please play {} manually)",
                    wav_path
                );
                continue;
            };
