...
pub fn MqttClient(comptime Socket: type, comptime Log: type, comptime Time: type) type {
    return struct {
        ...
        pub fn recvMessage(self: *Self, buf: []u8) ClientError!?Message {
            ...
            // Debug: log before recv
            Log.info("recvMessage: calling recv...", .{});

            const recv_len = self.recv(buf) catch |e| {
                // Debug: log the error
                Log.info("recvMessage: recv error, checking if Timeout...", .{});
                // Timeout is not an error for non-blocking recv
                if (e == error.Timeout) {
                    Log.info("recvMessage: was Timeout, returning null", .{});
                    return null;
                }
                Log.info("recvMessage: was NOT Timeout, returning RecvFailed", .{});
                return ClientError.RecvFailed;
            };

            Log.info("recvMessage: recv returned {d} bytes", .{recv_len});
        ...
        pub fn disconnect(self: *Self, buf: []u8) void {
            ...
            const disc_len = packet.encodeDisconnect(buf, .success) catch return;
        ...
        fn recv(self: *Self, buf: []u8) !usize {
            Log.info("mqtt recv: calling TLS recv...", .{});
            return self.socket.recv(buf) catch |e| {
                Log.info("mqtt recv: TLS recv returned error", .{});
