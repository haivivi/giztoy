...
pub const ReasonCode = enum(u8) {
    // Note: success,.success, and granted_qos_0 all share 0x00 in MQTT 5.0 spec
    // We use a single value and context-dependent interpretation
    success = 0x00,
    //.success = 0x00, // Same as success
    // granted_qos_0 = 0x00, // Same as success
    granted_qos_1 = 0x01,
    granted_qos_2 = 0x02,
...
pub fn decodeProperties(buf: []const u8) Error!struct { props: Properties, len: usize } {
    ...
    while (offset < end_offset) {
        ...
        switch (prop_id) {
            ...
            // Skip binary data properties
            @intFromEnum(PropertyId.correlation_data),
            @intFromEnum(PropertyId.auth_data),
            => {
                const bin_result = try decodeBinary(buf[offset..]);
                offset += bin_result.len;
            },
            // Skip user property (string pair)
            @intFromEnum(PropertyId.user_property) => {
                // Key
                const key_result = try decodeString(buf[offset..]);
                offset += key_result.len;
                // Value
                const val_result = try decodeString(buf[offset..]);
                offset += val_result.len;
            },
            // Skip variable int properties
            @intFromEnum(PropertyId.subscription_id) => {
                const var_result = try decodeVariableInt(buf[offset..]);
                offset += var_result.len;
            },
            // Skip 4-byte properties
            @intFromEnum(PropertyId.will_delay_interval) => {
                offset += 4;
            },
            // Skip 1-byte properties
            ...
            => {
                ...
                // Unknown property - cannot determine size, fail
...
pub fn encodeDisconnect(buf: []u8, reason_code: ReasonCode) Error!usize {
    ...
    if (reason_code == .success or reason_code == .success) {
        if (buf.len < 2) return Error.BufferTooSmall;
