...
// builtinJSONEncode implements __builtin.json_encode(value) -> string, error
...
// builtinJSONDecode implements __builtin.json_decode(str) -> value, error
func (rt *Runtime) builtinJSONDecode(state *luau.State) int {
	str := state.ToString(1)
	if str == "" {
		state.PushNil()
		state.PushString("empty string")
		return 2
	}
