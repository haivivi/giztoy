...
func TestClientServerPort_OpusFrames_ClientToServer(t *testing.T) {
	...
	// Give RecvFrom goroutines time to start
	...
	// Wait for frame to be processed and read from server port with timeout
	var readFrame opusrt.Frame
	var readErr error
	done := make(chan struct{})
	go func() {
		readFrame, _, readErr = serverPort.Frame()
		close(done)
	}()
